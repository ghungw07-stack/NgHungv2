...
export function makeURL(appContext, baseURL, params = {}, fillZpw = true) {
  ...
  if (fillZpw) {
    if (!url.searchParams.has("zpw_ver")) url.searchParams.append("zpw_ver", appContext.options.apiVersion);
    if (!url.searchParams.has("zpw_type")) url.searchParams.append("zpw_type", appContext.options.typeLogin);
  }
  return url.toString();
}
...
function getDefaultHeaders(appContext) {
...
export async function resolveResponse(appContext, res, cb) {
  const result = await handleZaloResponse(res, appContext);
...
export function apiFactory() {
  return (callback) => {
    return (api, appContext) => {
      if (!isContextHaveSecretKey(appContext))
        throw new ZaloApiError("Context is not have secret key: " + JSON.stringify(appContext, null, 2));
      const utils = {
      ...
      return callback(api, appContext, utils);
    };
  };
}
...
export async function getVideoMetadata(fileLink) {
  let metadata,
    isSuccess = true;
  const isRemoteLink = fileLink.startsWith("http://") || fileLink.startsWith("https://");

  try {
    metadata = await new Promise((resolve, reject) => {
      ffmpeg.ffprobe(fileLink, (err, data) => {
        if (err) reject(err);
        else resolve(data);
      });
    });
  ...
  if (metadata) {
    ...
    if (videoStream) {
      ...
      duration = videoStream.duration ? videoStream.duration * 1000 : 0;
      totalSize = metadata.format.size;
      if (!isRemoteLink) {
        const stats = fs.statSync(fileLink);
  ...
  if (!width || !height) {
    try {
      let imageMetadata;
      if (isRemoteLink) {
        imageMetadata = await sharp(fileLink, { animated: true }).metadata();
      } else {
        let fileHandle;
        try {
          fileHandle = await fs.promises.open(fileLink, "r");
          const fileData = await fileHandle.readFile();
          imageMetadata = await sharp(fileData).metadata();
        } finally {
          if (fileHandle) await fileHandle.close();
        }
      }
      width = imageMetadata.width;
      height = imageMetadata.pageHeight || imageMetadata.height;
    } catch (sharpErr) {
      console.error(`Sharp error with link ${fileLink} : `, sharpErr.message);
      // throw new Error(`Sharp error with link ${fileLink} : `, sharpErr.message);
      isSuccess = false;
    }
  }
...
export async function decodeEventData(parsed, cipherKey) {
  if (typeof parsed.data !== "string")
    throw new ZaloApiError(`Dữ liệu không hợp lệ, mong đợi kiểu string nhưng nhận được ${typeof parsed.data}`);
  if (typeof parsed.encrypt !== "number")
    throw new ZaloApiError(`Kiểu mã hóa không hợp lệ, mong đợi kiểu number nhưng nhận được ${typeof parsed.encrypt}`);
  if (parsed.encrypt < 0 || parsed.encrypt > 3)
    throw new ZaloApiError(`Kiểu mã hóa không hợp lệ, mong đợi giá trị 0-3 nhưng nhận được ${parsed.encrypt}`);
  ...
  if (encryptType !== 1) {
    if (cipherKey && decodedBuffer.length >= 48) {
      ...
      throw new ZaloApiError("Độ dài dữ liệu không hợp lệ hoặc thiếu cipher key");
    }
  }
...
export const logger = (appContext, obligatory = false) => ({
  verbose: (...args) => {
    if (appContext.options.showLogs || obligatory) console.log(`\x1b[35m 🔍 VERBOSE\x1b[0m`, `🟣`, ...args);
  },
  info: (...args) => {
    if (appContext.options.showLogs || obligatory) console.log(`\x1b[34m ℹ️ INFO\x1b[0m`, `🔵`, ...args);
  },
  warn: (...args) => {
    if (appContext.options.showLogs || obligatory) console.log(`\x1b[33m ⚠️ WARN\x1b[0m`, `🟡`, ...args);
  },
  error: (...args) => {
    if (appContext.options.showLogs || obligatory) console.log(`\x1b[31m ❌ ERROR\x1b[0m`, `🔴`, ...args);
  },
  success: (...args) => {
    if (appContext.options.showLogs || obligatory) console.log(`\x1b[32m ✅ SUCCESS\x1b[0m`, `🟢`, ...args);
  },
  timestamp: (...args) => {
    const now = new Date().toISOString();
    if (appContext.options.showLogs || obligatory)
      console.log(`\x1b[90m ⏰ [${now}]\x1b[0m`, ` {${appContext.phoneNumber || "_"}}`, ...args);
  },
});
...
export function getGroupEventType(act) {
  ...
  if (act == "new_group_invitation") return GroupEventType.NEW_INVITE_TO_GROUP;
  if (act == "remove_group_invitation") return GroupEventType.REMOVE_GROUP_INVITATION;
  return act;
}
export async function handleZaloResponse(response, appContext) {
  const result = {
    data: null,
    error: null,
  };

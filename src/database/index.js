...
async function loadConfig() {
  const configPath = path.join(JSON_DATA_PATH, "database-config.json");
...
export async function initializeDatabase() {
  try {
    ...
    const [tablesAccount] = await connection.execute(`SHOW TABLES LIKE '${NAME_TABLE_ACCOUNT}'`);
    ...
    const [tables] = await connection.execute(`SHOW TABLES LIKE '${NAME_TABLE_PLAYERS}'`);
    ...
    if (tables.length === 0) {
      ...
      const [columns] = await connection.execute(`SHOW COLUMNS FROM ${NAME_TABLE_PLAYERS}`);
      ...
      const requiredColumns = [
        ...
        {
          name: "serverId",
          query: "ADD COLUMN serverId VARCHAR(255) NOT NULL",
        },
        {
          name: "idUserZalo",

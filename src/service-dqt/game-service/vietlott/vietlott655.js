...
async function placeBet(api, message, threadId, senderId, amount, numbers) {
  ...
  if (numbers === "random") {
    ...
    if (
      ...
      playerNumbers.some((num) => isNaN(num) || num < 1 || num > 55) ||
      new Set(playerNumbers).size !== 6
    ) {
      await sendMessageFromSQL(api, message, {
        success: false,
...
Số phụ: ${result.extraNumber}`;
        ...
        await api.sendMessage(
          { msg: `Có lỗi xảy ra khi set kết quả: ${error.message}` },
          threadId,
...
export async function initializeGameVietlott655(api) {
  ...
  activeThreads = gameState.data.vietlott655.activeThreads;
  ...
  currentSession = {
    ...
    endTime:
      Date.now() +
      (Object.keys(gameState.data.vietlott655.players).length > 0 ? DEFAULT_INTERVAL : MAX_INTERVAL) * 1000,
...
async function toggleThreadParticipation(api, message, threadId, isStart) {
  const botId = api.getBotId();
  if (!gameState.data.vietlott655.activeThreads[botId]) gameState.data.vietlott655.activeThreads[botId] = [];
  const atcThreadofBot = gameState.data.vietlott655.activeThreads[botId];
  if (isStart) {
    if (!atcThreadofBot.includes(threadId)) {
      atcThreadofBot.push(threadId);
      saveGameData();
      await sendMessageFromSQL(
        api,
        message,
    ...
    const index = atcThreadofBot.indexOf(threadId);
    if (index > -1) {
      atcThreadofBot.splice(index, 1);
      saveGameData();
      await sendMessageFromSQL(
        api,
        message,
...
async function handleHistory(api, message, threadId) {
  ...
  gameHistory.forEach((result, index) => {
    ...
    const timeString = `${date.getHours().toString().padStart(2, "0")}:${date
      .getMinutes()
      .toString()
      .padStart(2, "0")}:${date.getSeconds().toString().padStart(2, "0")} ${date.getDate()}/${
      date.getMonth() + 1
    }/${date.getFullYear()}`;

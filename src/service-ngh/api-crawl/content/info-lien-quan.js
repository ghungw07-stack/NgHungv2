...
export async function createHeroInfoCard(dataHero, selectedSkin) {
  ...
  if (style !== 1) {
    ...
    return renderPortraitStyle(style, { kind: "profile", kicker: "MYBOT • LIÊN QUÂN DATABASE", title: selectedSkin.name, names: [dataHero.name || selectedSkin.name], avatars: [avatar], primaryLabel: "VAI TRÒ", primaryValue: dataHero.roles?.join(" / ") || "TƯỚNG", secondaryLabel: "KỸ NĂNG", secondaryValue: skills || "Đang cập nhật", body: selectedSkin.description || dataHero.lore || "Thông tin tướng và trang phục", footer: "Dữ liệu từ kho tướng" }, "hero-lienquan");
  }

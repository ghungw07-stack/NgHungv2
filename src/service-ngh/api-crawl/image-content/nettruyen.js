...
class NetTruyenScraper {
  constructor(ctx) {
    this.ctx = ctx;
    this.baseUrl = "https://nettruyenviet1.com";
    this.headers = () => {
      return {
        ...
        cookie: this.ctx.cookie.getCookieStringSync("https://nettruyenviet1.com"),
      };
    };
  ...
  async search(keyword) {
    const response = await this.axios.get(`${this.baseUrl}/Comic/Services/SuggestSearch.ashx`, {
      params: { q: keyword },
      headers: this.headers,
    });
    ...
    $("ul > li").each((_, el) => {
      ...
      results.push({
        ...
        chapter: i.eq(0).text().trim(),
        status: i.eq(1).find("b").text().trim(),
        genres: i.eq(2).text().trim().split(","),
      });
    });
    return results;
  }

  async getComicInfo(slugUrl) {
    ...
    let comicSlug = null,
      comicId = null,
      comicName = null;
    $detail('script[type="text/javascript"]').each((_, el) => {
      const scriptContent = $detail(el).html();
      if (scriptContent && scriptContent.includes("gOpts.comicSlug")) {
        const slugMatch = scriptContent.match(/gOpts\.comicSlug\s*=\s*'([^']+)'/);
        const idMatch = scriptContent.match(/gOpts\.comicId\s*=\s*'([^']+)'/);
        const nameMatch = scriptContent.match(/gOpts\.comicName\s*=\s*'([^']+)'/);
        if (slugMatch) comicSlug = slugMatch[1];
        if (idMatch) comicId = idMatch[1];
        if (nameMatch) comicName = nameMatch[1];
      }
    });
  ...
  async getChapterList(comicInfo) {
    this.headers.cookie = this.ctx.cookie.getCookieStringSync(this.baseUrl);
    const responseChapter = await this.axios.get(`${this.baseUrl}/Comic/Services/ComicService.asmx/ChapterList`, {
      params: {
        slug: comicInfo.slug,
    ...
    return responseChapter.data.data;
  }

  async getChapterImages(comicInfo, chapter) {
    this.headers.cookie = this.ctx.cookie.getCookieStringSync(this.baseUrl);
    const linkPage = `${this.baseUrl}/truyen-tranh/${comicInfo.slug}/${chapter.chapter_slug}`;
    ...
    const imageLinks = [];
    $chapter(".page-chapter img.lozad").each((_, el) => {
      const img = $chapter(el);
  ...
  async downloadImages(imageLinks, dir = tempDir) {
    const downloadPromises = imageLinks.map(async (imageUrl, idx) => {
      try {
        const imagePath = path.join(dir, `${idx}.jpg`);
...
const scraper = new NetTruyenScraper(ctx);

export const PLATFORM_NETTRUYEN = "nettruyen";
...
const listRequestNetTruyen = new Map();
const RELATED_EXPIRE_TIME = 300000;

schedule.scheduleJob("*/5 * * * * *", () => {
  const currentTime = Date.now();
  for (const [msgId, data] of listRequestNetTruyen.entries()) {
    if (currentTime - data.timestamp > CONFIG.timeWaitSelection) {
      listRequestNetTruyen.delete(msgId);
    }
  }
...
export async function handleNetTruyenCommand(api, message, aliasCommand) {
  ...
  try {
    ...
    if (dataFindComic && dataFindComic.length > 0) {
      ...
      //     stage: 1,
      //   };
      //   return await processNetTruyenStageReply(api, message, data, 0);
      // }
      ...
      listRequestNetTruyen.set(quotedMsgId.toString(), {
        userRequest: message.data.uidFrom,
        collection: dataFindComic,
        timestamp: Date.now(),
        stage: 1,
    ...
    let captErr = "Lỗi khi xử lý lệnh xem truyện tại NetTruyen, vui lòng liên hệ Admin để kiểm tra lỗi";
    console.error("Lỗi khi xử lý lệnh xem NetTruyen:", error.message);
...
export async function handleNetTruyenReply(api, message) {
  ...
  try {
    ...
    if (!listRequestNetTruyen.has(quotedMsgId)) return false;

    const dataComics = listRequestNetTruyen.get(quotedMsgId);
    ...
    listRequestNetTruyen.delete(quotedMsgId);

    await processNetTruyenStageReply(api, message, dataComics, selectedIndex);
    return true;
  } catch (error) {
    const captErr = "Lỗi khi xử lý lệnh xem truyện tại NetTruyen, vui lòng liên hệ Admin để kiểm tra lỗi";
    console.error("Lỗi khi xử lý phản hồi lệnh xem truyện tại NetTruyen:", error.message);
    await sendMessageFailed(api, message, captErr, false, 30000);
    return true;
  }
}

export async function processNetTruyenStageReply(api, message, dataRequest, selectedIndex) {
  const senderId = message.data.uidFrom;
  ...
  if (dataRequest.stage === 1) {
    ...
    try {
      ...
      const comicInfo = await scraper.getComicInfo(selectedComic.url);
      selectedComic.comicInfo = comicInfo;
      const chapterList = await scraper.getChapterList(comicInfo);
      ...
      if (keyMovieData.length === 1) {
        ...
        return await processNetTruyenStageReply(api, message, data, keyMovieData[0]);
      }
      ...
      console.error("Lỗi khi xử lý trạng thái phản hồi 1 của NetTruyen:", error);
      await api.addReaction("UNDO", message);
    ...
    try {
      ...
      const imageLinks = await scraper.getChapterImages(selectedComic.comicInfo, selectedSlug);
      ...
      const captErr = "Lỗi khi xử lý lệnh xem truyện tranh từ NetTruyen, vui lòng liên hệ Admin để kiểm tra lỗi";
      console.error("Lỗi khi xử lý trạng thái phản hồi 2 của truyện từ NetTruyen: ", error);
      await sendMessageFailed(api, message, captErr, false, 30000);
      await api.addReaction("UNDO", message);
      await api.addReaction("TIEUTAN", message);
    } finally {
...
export async function handleNextChapterNetTruyenReaction(api, reaction) {
  ...
  async sendMangaChapter({
    ...
    try {
      const caption =
        ...
        `\nFrom: Nettruyenvia.com (khuyến khích đọc truyện tại web để ủng hộ xem quảng cáo cho nguồn truyện tranh nhé)` +
      ...
      await sendReactionWaitingCountdown(api, msgSent, 15, PLATFORM_NETTRUYEN, fnAfterCountdown);
    } finally {
      if (tempFilePaths) {
        for (const item of tempFilePaths) {
          await deleteFile(item);

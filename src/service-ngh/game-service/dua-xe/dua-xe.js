...
function roomKey(botId, threadId) {
  return `duaxe:${normalizeId(botId)}:${normalizeId(threadId)}`;
}
...
async function recallLobby(api, room) {
  ...
  try {
    ...
    console.error("[DUA-XE] Không thu hồi được ảnh phòng cũ:", error?.message || error);
  }
}
...
async function sendLobbySnapshotInner(api, room, { recall = true } = {}) {
  ...
  let imagePath;
  try {
    imagePath = await createCarRaceLobbyImage({
    ...
    const sent = await api.sendMessage(
      {
        msg:
          `🏎️ PHÒNG ĐUA XE — cược ${formatCurrency(room.betAmount)} VNĐ/người\n` +
          `❤️ Thả tim ảnh này để vào phòng (${room.players.length}/${MAX_PLAYERS}).\n` +
          `👑 Chủ phòng dùng ${prefix}duaxe start để bắt đầu.`,
...
function scheduleLobbyTimeout(api, room) {
  ...
  room.timer = setTimeout(async () => {
    ...
    removeRoom(room);
    await sendGroupText(api, room, "⌛ Phòng đua xe đã tự hủy vì quá 5 phút chưa bắt đầu.", { ttl: 60_000 }).catch(
      () => {}
    );
  }, LOBBY_TIMEOUT_MS);
  room.timer.unref?.();
...
async function addPlayer(api, room, uid, fallbackName) {
  ...
  try {
    ...
    console.error("[DUA-XE] Lỗi thêm người chơi:", error);
...
async function createRoom(api, message, betText) {
  ...
  try {
    ...
    await reply(api, message, "❌ Số tiền cược không hợp lệ. Ví dụ: duaxe 10k hoặc duaxe 1m.");
    return;
  }
  ...
  try {
    ...
    removeRoom(room);
    console.error("[DUA-XE] Lỗi tạo phòng:", error);
    await reply(api, message, "❌ Không gửi được ảnh phòng đua xe lúc này.");
  }
}
...
async function settleRace(room, winner) {
  ...
  const totalPot = room.betAmount.mul(losers.length);
  const tax = totalPot.times(LIXI_RATE).round(0, Big.roundDown);    // 5% quỹ lì xì
  const winnerProfit = totalPot.minus(tax);                          // 95% cho người thắng
  const applied = [];
  ...
  try {
    ...
    for (const item of applied.reverse()) {
      ...
      if (!rollback?.success) console.error(`[DUA-XE] Không hoàn tác được số dư của ${item.uid}`);
    }
    throw error;
  }

  // Đóng 5% vào quỹ lì xì sau khi toàn bộ tiền đã chuyển thành công.
  if (tax.gt(0)) {
    await addToLuckyEnvelopeFund(tax).catch((error) => console.error("[DUA-XE] Lỗi cộng quỹ lì xì:", error));
  }
  ...
  return { winnerProfit, tax };
}

function buildResultMessage(room, ranking, winnerProfit, tax) {
  let msg = `🏁 KẾT QUẢ ĐUA XE\n💰 Cược: ${formatCurrency(room.betAmount)} VNĐ/người\n\n`;
  const mentions = [];
  const medals = ["🥇", "🥈", "🥉"];
  ...
  msg += `\n🏆 Người về nhất ăn 95% tổng cược.\n🧧 ${formatCurrency(tax)} VNĐ (5%) → Quỹ Lì Xì.`;
  return { msg, mentions };
}
...
async function startRace(api, message, room) {
  ...
  let paid = false;
  try {
    await sendGroupText(api, room, `🏎️ Đang đưa ${room.players.length} tay đua vào vạch xuất phát...`, {
      quote: message,
      ttl: 60_000,
    });
    const race = await createCarRaceGif(room.players);
    gifPath = race.gifPath;
    const { winnerProfit, tax } = await settleRace(room, race.ranking[0]);
    paid = true;
    ...
    const result = buildResultMessage(room, race.ranking, winnerProfit, tax);
    await sendGroupText(api, room, result.msg, { mentions: result.mentions });
    removeRoom(room);
  } catch (error) {
    console.error("[DUA-XE] Lỗi bắt đầu cuộc đua:", error);
    if (paid) {
      removeRoom(room);
...
async function leaveRoom(api, message, room) {
  ...
  if (senderId === room.hostId) return reply(api, message, "⚠️ Chủ phòng hãy dùng duaxe huy để đóng phòng.");
...
async function cancelRoom(api, message, room) {
  ...
  await recallLobby(api, room);
  removeRoom(room);
  await reply(api, message, "✅ Đã hủy phòng đua xe.", 60_000);
}

function guide(prefix) {
  return (
    `🏎️ ĐUA XE — tiền ảo, không quy đổi tiền mặt\n\n` +
    `• ${prefix}duaxe <tiền>: tạo phòng (VD: ${prefix}duaxe 10k)\n` +
    `• Thả ❤️ vào ảnh phòng để tham gia\n` +
    `• ${prefix}duaxe start: chủ phòng bắt đầu\n` +
    `• ${prefix}duaxe roi: rời phòng\n` +
    `• ${prefix}duaxe huy: hủy phòng\n\n` +
    `Từ 2–${MAX_PLAYERS} người; người về nhất ăn 95% tổng cược, 5% vào quỹ lì xì.`
  );
}

export async function handleCarRaceCommand(api, message) {
  const prefix = getGlobalPrefix(api.getBotId());
  if (message.type !== MessageType.GroupMessage) {
    await reply(api, message, "⚠️ Lệnh đua xe chỉ sử dụng trong nhóm.");
    return;
  }
  ...
  if (["start", "batdau"].includes(sub)) {
    if (!room) return reply(api, message, `⚠️ Chưa có phòng. Tạo bằng ${prefix}duaxe 10k.`);
    return startRace(api, message, room);
  }
  if (["huy", "cancel"].includes(sub)) {
    if (!room) return reply(api, message, "⚠️ Nhóm chưa có phòng đua xe.");
...
export async function handleCarRaceReaction(api, reaction) {
...
export const __carRaceDebug = { rooms, lobbyReactions, recentReactions };

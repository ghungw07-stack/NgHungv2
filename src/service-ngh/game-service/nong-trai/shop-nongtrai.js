...
import { checkItemQuantity, setHasChange, TIME_TO_LIVE, updatePlayerBagHasSetting } from "./nong-trai.js";
...
async function expandFarm(botId, farm) {
  const newPlot = {
    crop: null,
    plantedAt: null,
    waterLevel: 100,
...
export async function handleBuyProduct(api, message, farm, buyProductCode, quantity) {
  ...
  if (product.type === "ITEM") {
    if (product.key === "LAND") {
      ...
      if (buyResult.success) {
        await updatePlayerBagHasSetting(botId, farm, buyProductCode, quantity);
      }
      await api.sendMessage({ msg: buyResult.message, quote: message, ttl: TIME_TO_LIVE }, threadId, message.type);
    }
...
async function sellProduct(botId, farm, senderId, productCode, quantity = 1) {
  ...
  await updatePlayerBagHasSetting(botId, farm, productCode, -sellQuantity);
...
export async function handleSellProduct(api, message, farm, args) {
  ...
  const sellResult = await sellProduct(botId, farm, senderId, sellProductCode, sellQuantity);

...
test("learnnow preserves legacy underscore syntax", async () => {
  let saved;
  const registry = new CommandRegistry();
  registerAutoReplyCommands(registry, { repository: { async set(...args) { saved = args; } }, settings: {} });
  await registry.resolve("learn").execute({ args: [], content: "!learnnow_xin chào_chào bạn", prefix: "!", invokedName: "learnnow", threadId: "g", type: 1, senderId: "u", reply: async () => {} });
...
test("auto reply event ignores disabled groups and throttles duplicate triggers", async () => {
  ...
  registerAutoReplyEvents(eventBus, {
    settings: { async get() { return { learnEnabled: enabled }; } },
